...
// Function to epdate the tile's appearance based on the num value
...
// event that triggers the the webpage finishes loading
...
// "e" represents an event, which contains an information about the action occurred
...
function slide(row){
	...
	for(let i=0; i < row.length - 1; i++){
		if(row[i] == row[i+1]){
			...
			row[i] *= 2;
			// and setting the second element to 0
			row[i+1] = 0;
			// logic for scoring
			score += row[i];
			// [2,2,2] -> [4,0,2]
...
function slideRight(){
	...
	for(let r=0; r < rows; r++){
		...
		// calling the slide function to merge the tiles
		row = slide(row);
		...
		// update the id of the tiles as well as the appearance
		for(let c=0; c < columns; c++){
			let tile = document.getElementById(r.toString() + "-" + c.toString());
			let num = board[r][c];
...
function slideUp(){
	for(let c = 0; c < columns; c++) {
        ...
        // check with tiles have changed in column
        let changedIndices = [];

        for(let r=0; r < rows; r++){
        	// this will record the current position that have changed
...
function setTwo(){
	...
	while(!found){
		// Math.random() - used to generate a number between 0 - 1 times to the rows/columns
...
document.addEventListener('touchmove', (e)=>{
	if(!e.target.className.inclused("tile")){
		return 
	}
...
document.addEventListener("touchend", (e) =>{
	// check if the element triggered event has a classname tile
